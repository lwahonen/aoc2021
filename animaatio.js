...
var file = "";
if (isNode) {
    file = (0, libraries_1.fetchInputData)(year, day);
}
else {
    var sync_fetch = require('sync-fetch');
    file = sync_fetch("data/day_".concat(day, ".txt")).text();
}
///////////////////////////////////////////////////
// START HERE
///////////////////////////////////////////////////
var rows = file.split("\n").map(function (f) { return f.match(/position=<\s*([-]?\d+),\s*([-]?\d+)> velocity=<\s*([-]?\d+),\s*([-]?\d+)/); }).filter(function (f) { return f != null; });
var nodes = rows.map(function (f) {
    var x = parseInt(f[1]);
    var y = parseInt(f[2]);
    var dx = parseInt(f[3]);
    var dy = parseInt(f[4]);
...
var tick = function (t) {
    ...
    nodes.forEach(function (el) {
        var x = (el.x - min_x) / (max_x + 10);
        var y = (el.y - min_y) / (max_y + 10);
        var px = x * width;
        var py = y * height;
        if (ctx != undefined) {
            ctx.fillStyle = 'white';
...
},{"./libraries":2,"detect-node":4,"sync-fetch":6}],2:[function(require,module,exports){
"use strict";
exports.__esModule = true;
...
function fetchInputData(year, day) {
    var path1 = "/Users/lwahonen/Dropbox/advent/2018/data/day_".concat(day, ".txt");
    if (fs.existsSync(path1)) {
        // File exists in path
    }
    else {
        var exec = require("child_process").exec;
        var command = "/Users/lwahonen/aocli/aocdl -year ".concat(year, " -day ").concat(day, " -output ").concat(path1);
...
},{"child_process":8,"fs":8}],3:[function(require,module,exports){
'use strict'
...
},{"base64-js":3,"buffer":10,"ieee754":5}],8:[function(require,module,exports){
